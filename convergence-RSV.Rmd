---
title: "Converging sequences - RSV"
author: "Joseph Ng"
date: "14 June 2021"
output: 
  html_document:
...
knitr::opts_chunk$set(
	echo = TRUE, message = FALSE, warning = FALSE, fig.path = "ConvergenceRSV_plots/", 
	dev = "svg"
)
...
* All UseAsRef sequences from RSV-I and RSV-U samples
...
pdb_ab <- read.table('known_binders/20210519ver_sabdab_RSVAbs_sequences_VJgenes.tsv', 
...
pdb_ab$source <- "PDB"
pdb_ab$Specificity <- factor(pdb_ab$Specificity, levels = c("F", "G"))#,
#                             labels = c("RSV-F", "RSV-G"))
...
cloned_ab_pub <- c("known_binders/Cortjens_et_al_RSV-G.txt",
...
cloned_ab_pub[[1]]$source <- "Cortjens_et_al"
cloned_ab_pub[[2]]$source <- "Goodwin_et_al"
cloned_ab_pub[[3]]$source <- "Gilman_et_al"
cloned_ab_pub[[1]]$Specificity <- "G"
cloned_ab_pub[[2]]$Specificity <- "F"
cloned_ab_pub[[3]]$Specificity <- "F"
...
saveRDS(converge, "ConvergeSequences_RSVAbPDB_graph_SameVJPatient.rds")

```

## Known binders

Added to this set of sequences known binders of RSV proteins from the following sources:

* CDR3s from antibody structures resolved in complex with RSV proteins (Search PDBe 21st May 2021). Binding either F or G protein (but could target different epitopes).

*. Cloned & Experimentally confirmed binders to RSV proteins from these publications:

1. [Cortjens et al 2017 J Virol](https://journals.asm.org/doi/full/10.1128/JVI.02357-16) - Cloned and tested (small) panel of antibodies targeting RSV G protein.
2. [Gilman et al 2016 Sci Immunol](https://dx.doi.org/10.1126/sciimmunol.aaj1879) RSV F specific B cells isolated from naturally infected adult donors and cloned antibodies to test for specificity.
3. [Goodwin et al 2018 Immunity](https://doi.org/10.1016/j.immuni.2018.01.005) RSV F specific B cells isolated from naturally infected infants and cloned antibodies to test.

If V & J germline gene assignment are missing from data source they are annotated using either IMGT/High-VQuest (if nucleotide sequences are available) or IMGT/DomainGapAlign (if only amino acid sequences are available). **Total: 737 unique CDRH3 sequences from PDB and publications**.
...
converge <- readRDS("ConvergeSequences_RSVAbPDB_graph_SameVJPatient.rds")
convg_data <- do.call("rbind", convg_data)
...
saveRDS(membership, 'convergent_cluster_membership_RSV.rds')
```
...
g1 <- ggplot(cluster_large, aes(x = cluster, y = V1, fill = SampleType2)) + geom_bar(stat = "identity") +
  ...
  scale_fill_manual(values = c("Healthy" = "#7FC97F", "RSV-U" = "#F0027F", "RSV-I" =  "#386CB0",
                               "F" = "#99CCFF"), name = "") +
  xlab("Convergent clusters") +ylab("Number of\nsequences") + ggtitle("Clusters with 10 or more sequences (n = 9)")
# Vgene usage
...
# similarity between sequences within the same cluster
converge <- merge(converge, membership[, c("Seq_ID", "cluster")], by.x = "Seq1", 
                  by.y = "Seq_ID", all.x = TRUE, all.y = FALSE, sort = FALSE)
converge_large <- converge[which(converge$cluster %in% cluster_large$cluster), ]
converge_large$cluster <- factor(converge_large$cluster, levels = levels(cluster_large$cluster))
g4 <- ggplot(converge_large, aes(x = cluster, y = identity)) + 
  stat_summary(fun.min = min, fun.max = max, fun = median) + xlab("Convergent clusters") + 
  scale_y_continuous(labels = scales::percent, name="CDRH3 AA\nidentity (%)") +
  cowplot::theme_cowplot() + theme(axis.text.x = element_blank(),
                                   axis.ticks.x = element_blank()) 

cowplot::plot_grid(g1, g2, g3, #g4, 
                   ncol = 1, axis = "lr", align = "v", rel_heights = c(2, 2, 1.3))#, 1))
```
...
We have a few clusters of sequences recognising the F protein.
...
visualiseNetwork <- function(graph, nodes, node_annotation, cluster, node_id_col = "Seq_ID", 
                             ...
                             node_colors = c("Healthy" = "#7FC97F", "RSV-U" = "#F0027F", 
                                             "RSV-I" =  "#386CB0", "F" = "#99CCFF"))
{
...
visualiseNetwork(converge_graph, membership[membership$cluster == 498, 1], cluster = 498,
                 node_annotation = converge_nodes)
visualiseNetwork(converge_graph, membership[membership$cluster == 520, 1], cluster = 520,
                 node_annotation = converge_nodes)
```
...
seqs <- membership[membership$cluster %in% c(498, 520), c("Seq_ID", "cluster", "CDRH3")]
...
cowplot::plot_grid(
  # cluster 498
  ggseqlogo(seqs[which(grepl("Cluster 498", seqs$cluster)), "CDRH3"]) +
    ylab(unique(seqs[which(grepl("Cluster 498", seqs$cluster)), "cluster"])) +
  ...
  # cluster 520
  ggseqlogo(seqs[which(grepl("Cluster 520", seqs$cluster)), "CDRH3"]) +
    ylab(unique(seqs[which(grepl("Cluster 520", seqs$cluster)), "cluster"])) +
    theme_cowplot() + theme(legend.position = "none", axis.text = element_blank(), axis.ticks = element_blank(),
                            axis.line = element_blank(), axis.title.y = element_text(angle = 0, vjust = 0.5)),
