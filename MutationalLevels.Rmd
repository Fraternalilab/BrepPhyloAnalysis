...
distFromGermline_AUC$SampleType <- factor(distFromGermline_AUC$SampleType,
                                          levels = c("Healthy", "COVID", "EB",
                                                     "Infected",
                                                     "Uninfected", "YF"),
                                          labels = c("Healthy", "COVID-19", "Ebola",
                                                     "RSV-I", "RSV-U", "YFVD28"))
...
cowplot::plot_grid(
  ggplot(distFromGermline_AUC_all[which(distFromGermline_AUC_all$SampleType2 %in% c("Healthy", "Ebola")),], aes(x = SampleType2, y = AUC)) + 
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  geom_boxplot(outlier.shape = NA) + geom_point() + ylim(0, 1) +
  cowplot::theme_cowplot() + xlab("") + ylab("Germline likeness") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        plot.margin = margin(10, 2, 2, 2), legend.position = "bottom"),
  ggplot(distFromGermline_AUC[which(distFromGermline_AUC$SampleType == "COVID-19"), ], 
...
print(DescTools::JonckheereTerpstraTest(AUC ~ TimePoint, data = distFromGermline_AUC[which(distFromGermline_AUC$SampleType == "COVID-19"), ]))
```
...
Does this correlate with any metadata?

```{r, fig.width=8}
cv_metadata <- read.table('CV_metadata.txt', header = TRUE,
                          stringsAsFactors = FALSE, sep = "\t")
cv_metadata <- merge(cv_metadata, 
                     distFromGermline_AUC[which(distFromGermline_AUC$TimePoint == "0"), c("PatientId", "AUC")],
                     by.x = "Patient", by.y = "PatientId")
cv_metadata_grouped <- reshape2::melt(cv_metadata, id.vars = c("Patient", "AUC"),
                                      measure.vars = c("SeverityScore", "Gender", "Age.group",
                                                       "Oxygen.required.",
                                                       "MADU.", "ICU."))
cv_metadata_grouped$value <- factor(cv_metadata_grouped$value,
                                    levels = c("M", "F", "Young", "Middle", "Older",
                                               "Y", "N", "1", "2", "3", "4", "5", "6"))
cv_metadata_grouped <- cv_metadata_grouped[which(cv_metadata_grouped$value != ""), ]

cowplot::plot_grid(
  cowplot::plot_grid(
    ggplot(cv_metadata, aes(x = CRP.Levels..mg.L., y = AUC)) +
      xlab("CRP (mg/L)") + ylab("Germline likeness (Day 0)") +
      geom_point() + ggpubr::stat_cor(method = "spearman") +
      cowplot::theme_cowplot(),
    ggplot(cv_metadata, aes(x = Days.since.symptom.onset, y = AUC)) +
      xlab("Days since symptom onset") + ylab("Germline likeness (Day 0)") +
      geom_point() + ggpubr::stat_cor(method = "spearman", 
                                      label.x = 14, label.y = 0.7) +
      cowplot::theme_cowplot(),
    nrow = 1, ncol = 2, axis = "tb", align = "h"
  ),
  ggplot(cv_metadata_grouped, aes(x = value, y = AUC)) +
    geom_boxplot() + geom_point() + ylab("Germline likeness (Day 0)") + xlab("") +
    cowplot::theme_cowplot() + facet_wrap(~ variable, scales = "free_x", nrow = 1) +
    theme(axis.text.x = element_text(size = 9)),
  nrow = 2, ncol = 1, axis = "lr", align = "v"
)
```
...
ggplot(distFromGermline_AUC, aes(x = Class, y = AUC, colour = sig)) +
  ...
  geom_hline(yintercept = 0.5, linetype = "dashed") + geom_boxplot() +
  geom_point(position = position_dodge()) + 
  scale_colour_manual(values = c("TRUE" = "red", "FALSE" = "grey"), 
                      name = "p < 0.05") + ylab("Germline likeness") +
